...
def setup_create_ui(self):
    self.on_after_component_elem_id.append(('txt2img_hires_fix_row2', partial(create_ui_cfg, self)))
    self.on_after_component_elem_id.append(('txt2img_hires_fix_row4', partial(create_ui_hr_prompt_mode, self)))
...
def create_ui_hr_prompt_mode(self, *args, **kwargs):
    ...
    with gr.Row():
        self.hr_prompt_mode_e = gr_ui_element(choices=list(hr_prompt_mode.hires_prompt_mode_functions), label='Hires prompt mode', value='Default', elem_id=self.elem_id('hr_prompt_extend_mode'), visible=shared.opts.hires_fix_show_prompts and shared.opts.hires_fix_tweaks_show_hr_prompt_mode)
        self.hr_negative_prompt_mode_e = gr_ui_element(choices=list(hr_prompt_mode.hires_prompt_mode_functions), label='Hires negative prompt mode', value='Default', elem_id=self.elem_id('hr_negative_prompt_extend_mode'), visible=shared.opts.hires_fix_show_prompts and shared.opts.hires_fix_tweaks_show_hr_prompt_mode)
...
def create_hr_seed_ui(self, *args, **kwargs):
    if self.create_hr_seed_ui_done:
        return
    with ui_components.InputAccordion(False, label="Hr Seed", elem_id=self.elem_id('custom_seed')) as self.enable_hr_seed_e:
        ...
        self.on_after_component(lambda x: connect_reuse_seed(self.hr_seed_e, reuse_seed, x.component, False), elem_id=f'html_info_{self.tabname}')
        self.on_after_component(lambda x: connect_reuse_seed(self.hr_subseed_e, reuse_subseed, x.component, True), elem_id=f'html_info_{self.tabname}')
